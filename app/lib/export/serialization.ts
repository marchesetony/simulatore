...
function contractualHtml(proposal: ProposalCanonicalSnapshot): string {
  if (!proposal.contractualPassThrough) return "";
  const rows = proposal.contractualPassThrough.states.map((state) => `<tr><td>${escapeHtml(state.kind)}</td><td>${escapeHtml(state.state)}</td><td>${escapeHtml(state.effectiveFrom)} to ${escapeHtml(state.effectiveTo)}</td></tr>`).join("");
  return `<section><h2>Contractual pass-through</h2><p>Completeness: ${escapeHtml(proposal.contractualPassThrough.completeness)}; BTA6 complete: ${escapeHtml(proposal.contractualPassThrough.bta6NetOfTaxComplete)}</p><table><thead><tr><th>Kind</th><th>Status</th><th>Period</th></tr></thead><tbody>${rows}</tbody></table></section>`;
}
