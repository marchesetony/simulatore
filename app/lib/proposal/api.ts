...
export async function exportResponse(body: Record<string, unknown>, tenantId: string, format: ProposalExportFormat, principal?: AuthenticatedPrincipal): Promise<Response> {
  ...
  const proposal = body.proposal as { readonly proposalId: string; readonly proposalFingerprint: string };
  const contentFingerprint = createHash("sha256").update(document.body, "utf8").digest("hex");
  await runtimeRepositories().exports.put({ tenantId, recordId: `export_${contentFingerprint.slice(0, 32)}`, payload: { exportId: `export_${contentFingerprint.slice(0, 32)}`, proposalId: proposal.proposalId, format, contentFingerprint }, idempotencyKey: contentFingerprint });
