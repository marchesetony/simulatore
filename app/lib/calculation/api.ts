...
export function calculationError(error: unknown, fallback = "CALCULATION_REQUEST_INVALID"): Response {
  ...
  const status = code === "TENANT_ACCESS_DENIED" ? 403 : code.endsWith("NOT_FOUND") ? 404 : code.startsWith("SOURCE_BILL_") || code.includes("INCOMPATIBLE") || code.includes("MISSING") ? 409 : 400;
