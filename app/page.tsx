...
export default function Home() {
  ...
  const gestisciUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files?.[0]) return;
    setCaricamento(true);
    setRisultato(null);

    // Simulazione del processo OCR con Intelligenza Artificiale
    setTimeout(() => {
      setCaricamento(false);
      setRisultato("✅ CTE letta con successo! Offerta: 'Business Chiara GME' - Target: B2B - Spread estratto: 0,015 €/kWh.");
    }, 3000);
  };
      ...
      <div className="max-w-2xl w-full bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
        ...
        <div className="border-2 border-dashed border-blue-200 rounded-xl p-8 flex flex-col items-center bg-blue-50/30 hover:bg-blue-50/60 transition-colors">
          ...
          <label className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-6 rounded-lg shadow transition-colors">
            {caricamento ? 'Elaborazione PDF con AI...' : 'Seleziona PDF Scheda Tecnica'}
            <input type="file" accept="application/pdf" className="hidden" onChange={gestisciUpload} disabled={caricamento} />
          </label>
