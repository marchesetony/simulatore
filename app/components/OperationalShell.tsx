...
import BillOperationalPanel from "./BillOperationalPanel";
...
function LegacyBillsPanel({ readonly }: { readonly readonly: boolean }) {
...
function BillsPanel({ readonly, onUseInSimulation }: { readonly readonly: boolean; readonly onUseInSimulation: (draft: SimulationDraft) => void }) { return <BillOperationalPanel readonly={readonly} onUseInSimulation={onUseInSimulation} />; }
...
export default function OperationalShell() {
  ...
  function useBillInSimulation(value: SimulationDraft) { setSimulationPrefill(value); setDraft(value); setSection("simulations"); } function onCalculation(result: CalculationModel, value: SimulationDraft) { setCalculation(result); setComparison(null); setSelectedId(result.calculationId); setDraft(value); } function onComparison(result: ComparisonModel, value: SimulationDraft) { setComparison(result); setCalculation(null); setSelectedId(result.ranking[0]?.calculationId ?? null); setDraft(value); }
  return <div className="operational-app"><a className="skip-link" href="#main-content">Vai al contenuto</a><header className="app-header"><div className="brand-lockup"><div className="brand-mark" aria-hidden="true">E</div><div><p className="eyebrow">FOUNDATION V1  OPERATIONS</p><h1>Energia Operativa</h1></div></div><div className="header-context"><span className="mode-pill">{context?.authSource === "LOCAL_SYNTHETIC" ? "Locale sintetico esplicito" : context?.runtimeMode === "production" ? "Production" : "Contesto non disponibile"}</span><span className="mode-pill">{context?.role ?? "Sola lettura"}</span></div></header><div className="app-layout"><aside className="app-sidebar"><nav aria-label="Navigazione principale"><p className="nav-title">Aree operative</p>{sections.map((item) => <button key={item.id} className={section === item.id ? "nav-item active" : "nav-item"} type="button" onClick={() => setSection(item.id)} aria-current={section === item.id ? "page" : undefined}><span className="nav-icon" aria-hidden="true">{item.short}</span><span>{item.label}</span></button>)}</nav><div className="sidebar-note"><strong>{readonly ? "Vista sola lettura" : "Azioni operative"}</strong><span>Ruoli, tenant e permessi restano server-authoritative.</span></div></aside><main id="main-content" className="app-main" tabIndex={-1}>{section === "dashboard" ? <DashboardPanel context={context} readiness={readiness} readinessState={readinessState} /> : null}{section === "bills" ? <BillsPanel readonly={readonly} onUseInSimulation={useBillInSimulation} /> : null}{section === "cte" ? <CteIngestionPanel readonly={readonly} /> : null}{section === "market" ? <MarketPanel readonly={readonly} /> : null}{section === "simulations" ? <><SimulationPanel tenantId={tenantId} readonly={readonly} prefill={simulationPrefill} onCalculation={onCalculation} onComparison={onComparison} /><ResultCards calculation={calculation} comparison={comparison} selectedId={selectedId} onSelect={setSelectedId} /></> : null}{section === "proposals" ? <ProposalsPanel calculation={calculation} comparison={comparison} selectedId={selectedId} draft={draft} tenantId={tenantId} readonly={readonly} /> : null}{section === "system" ? <SystemPanel context={context} readiness={readiness} onReload={() => { void loadContext(); void loadReadiness(); }} /> : null}</main></div><footer className="app-footer"><span>Dati authoritative o esplicitamente non disponibili</span><span>Nessun token o segreto persistito nel browser</span></footer></div>;
}
