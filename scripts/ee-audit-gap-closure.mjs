...
const applicableInJuly = (record) => Date.parse(record.effectiveFrom) <= Date.parse(PERIOD_FROM) && (record.effectiveTo === null || Date.parse(PERIOD_FROM) < Date.parse(record.effectiveTo));
const applicableDomain = (records, domain) => records.filter((record) => applicableInJuly(record) && referenceDomainOf(record) === domain);
...
const gmeReferences = market ? [{ month: "2026-07", f1: market.f1?.value ?? null, f2: market.f2?.value ?? null, f3: market.f3?.value ?? null, unit: "EUR/MWH", sourceReference: market.source.url, officialIdentifier: market.recordId }] : [];
...
const arera587 = regulatory.filter((item) => item.officialIdentifier === "587/2025/R/eel" && applicableInJuly(item));
...
const statusFor = (code, fallbackCode = code) => phantom.get(code) ?? (applicableDomain(regulatory, "DISPATCHING").some((record) => record.componentCode === fallbackCode) ? "OFFICIAL_REFERENCE_AVAILABLE" : "SOURCE_DISCOVERY_REQUIRED");
...
console.log(`PUN_APPLIED_DISPLAY_VALUE=${matrix.pun.appliedDisplayValue}`);
console.log(`PUN_APPLIED_DISPLAY_UNIT=${matrix.pun.appliedDisplayUnit}`);
for (const band of matrix.pun.source) {
  console.log(`GME_${band.band}_DISPLAY_VALUE=${band.displayValue}`);
  console.log(`GME_${band.band}_DISPLAY_UNIT=${band.displayUnit}`);
}
console.log(`AMOUNT_UNIT_ISSUES=${amountUnitIssues}`);
console.log(`TOTAL_CONFIRMED_ANOMALIES=${totalConfirmedAnomalies}`);
console.log(`ZERO_ANOMALIES_RENDERED_AS_REGULAR=${totalConfirmedAnomalies === 0 && overallBillAuditability !== "COMPLETE" ? "NO" : "NOT_APPLICABLE"}`);
console.log("LEGACY_TERNA_COUNT_USED_FOR_COVERAGE=NO");
...
const authoritySeparateFromDomain = dispatching.length === dispatchingAuthorityCounts.ARERA + dispatchingAuthorityCounts.TERNA && dispatchingAuthorityCounts.ARERA > 0;
const arera587Classification = arera587.length === 2 && arera587.every((item) => referenceDomainOf(item) === "DISPATCHING" && item.authority === "ARERA");
const capacityOffPeakRealStructure = capacity.length === 1 && capacityReference?.componentCode === "CAPACITY_MARKET_OFF_PEAK" && referenceDomainOf(capacityReference) === "CAPACITY_MARKET" && !capacity.some((item) => ["CAPACITY_MARKET", "CAPACITY_MARKET_PEAK"].includes(item.componentCode));
const sourceCoverageSeparateFromAuditability = matrix.coverage.ARERA_NETWORK_SOURCE_COVERAGE === "VERIFIED" && matrix.coverage.ARERA_NETWORK_BILL_AUDITABILITY === "DOCUMENT_DETAIL_REQUIRED";
const punEurKwh = matrix.pun.appliedDisplayValue === 0.196201 && matrix.pun.appliedDisplayUnit === "EUR/KWH"
  && JSON.stringify(matrix.pun.source.map((band) => [band.band, band.displayValue, band.displayUnit])) === JSON.stringify([["F1", 0.1542, "EUR/KWH"], ["F2", 0.16938, "EUR/KWH"], ["F3", 0.15226, "EUR/KWH"]]);
const zeroAnomaliesNotRegular = totalConfirmedAnomalies === 0 && overallBillAuditability !== "COMPLETE" && (totalConfirmedAnomalies === 0 ? "NO" : "NOT_APPLICABLE") === "NO";

console.log(`GIT_DIFF_CHECK=${await commandStatus(process.platform === "win32" ? "git.exe" : "git", ["diff", "--check"])}`);
console.log(`TSC=${await commandStatus(process.platform === "win32" ? "npx.cmd" : "npx", ["tsc", "--noEmit"])}`);
...
console.log(`ZERO_ANOMALIES_NOT_REGULAR_WHEN_PARTIAL=${zeroAnomaliesNotRegular ? "PASS" : "FAIL"}`);
