...
const makeRecord = (domain, value, period = ["2026-01-01", "2027-01-01"], sourceSha = "b") => createRegulatoryValue({ tenantId: tenant, sourceType: "OFFICIAL_ATTACHMENT", sourceReference: "https://www.arera.it/qa/regulatory-refresh.xlsx", officialIdentifier: `QA-REFRESH-${domain.componentCode}-${domain.normalizedUnit}-${domain.regulatoryVariant ?? "LEGACY"}`, publicationDate: "2026-01-01", retrievedAt: now, effectiveFrom: period[0], effectiveTo: period[1], componentCode: domain.componentCode, customerScope: domain.customerScope, ...(domain.regulatoryVariant === undefined ? {} : { regulatoryVariant: domain.regulatoryVariant }), originalValue: value, originalUnit: domain.normalizedUnit, applicationBasis: "QA fixture; official ARERA domain", sourceSha256: sourceSha.repeat(64).slice(0, 64) });
