...
try {
  ...
  try {
    ...
    registerProductionSessionAdapter({ resolve: () => ({ userId: "user_provider", tenantId: "tenant_auth-smoke", role: "ANALYST", sessionId: "session_provider", issuedAt: "2026-01-01T00:00:00.000Z", expiresAt: "2099-01-01T00:00:00.000Z", source: "VERIFIED_SESSION" }) });
    const providerRecords = { billIngestionMetadata: adapter.collection("provider-bill-ingestion"), normalizedBillSnapshots: adapter.collection("provider-bill-snapshots"), cteArchives: adapter.collection("provider-cte"), marketDataArchives: adapter.collection("provider-market"), regulatoryValues: adapter.collection("provider-regulatory-values"), approvalDomains: adapter.collection("provider-approval-domains"), calculationResults: adapter.collection("provider-calculations"), comparisonResults: adapter.collection("provider-comparisons"), proposals: adapter.collection("provider-proposals"), exports: adapter.collection("provider-exports"), auditEvents: adapter.collection("provider-audit") };
